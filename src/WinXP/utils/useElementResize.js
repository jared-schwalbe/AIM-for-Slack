...
import nsResize from '../assets/cursor/ns-resize.png';
import ewResize from '../assets/cursor/ew-resize.png';
import nwseResize from '../assets/cursor/nwse-resize.png';
import neswResize from '../assets/cursor/nesw-resize.png';
...
function useElementResize(ref, options) {
  ...
  useEffect(() => {
    ...
    function onMouseDown(e) {
      ...
      if (e.target !== target || !resizable) {
        return;
      }

      switch (cursorPos) {
        case 'topLeft':
          _boundary.right = originMouseX + previousSize.width - constraintSize.width;
          _boundary.bottom =
            originMouseY + previousSize.height - constraintSize.height;
        ...
        case 'bottomLeft':
          _boundary.right = originMouseX + previousSize.width - constraintSize.width;
          _boundary.top = originMouseY - previousSize.height + constraintSize.height;
        ...
        case 'topRight':
          _boundary.bottom =
            originMouseY + previousSize.height - constraintSize.height;
          _boundary.left = originMouseX - previousSize.width + constraintSize.width;
          onDragStartTop(e);
...
function useCursor(ref, threshold, resizable) {
  ...
  useEffect(() => {
    ...
    function _setPosition(p) {
      ...
      document.getElementById('win-xp').style.cursor = getCursorStyle(p);
    }

    function onMouseDown(e) {
      const headerClicked = e.target.className === 'app__header';
      if (e.which !== 1 || (e.target !== target && !headerClicked)) {
        return;
      }
    ...
    function onMouseUp(e) {
      lock = false;
      cover.remove();
      document.getElementById('win-xp').style.cursor = getCursorStyle(position);
